...
export default function BudgetDetailPage() {
  ...
  const loadBudget = useCallback(() => {
    ...
    if (storedBudgets) {
      ...
      if (currentBudget) {
        // Recalculate totalBudgetedAmount on load
        currentBudget.totalBudgetedAmount = currentBudget.categories.reduce((sum, cat) => sum + (cat.budgetedAmount || 0), 0);
  ...
  const updateBudgetInList = (updatedBudget: Budget) => {
    ...
    if (budgetIndex > -1) {
      ...
      budgets.push(updatedBudget); // Should not happen if loading existing
    }
    saveBudgetsToLocalStorage(budgets);
    setBudget(updatedBudget); // Update current page state
  };
  ...
  const handleCategorySubmit: SubmitHandler<CategoryFormValues> = (data) => {
    ...
    if (editingCategory) {
      ...
      const newCategory: BudgetCategory = {
        ...
        ...data,
        lineItems: [], // Initialize line items for future use
      };
    ...
    <div>
      ...
      <Card className="mb-6 shadow-lg">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div><span className="font-semibold text-muted-foreground">Name:</span> {budget.name}</div>
          <div><span className="font-semibold text-muted-foreground">Period:</span> {format(parseISO(budget.startDate), 'PP')} to {format(parseISO(budget.endDate), 'PP')}</div>
          <div className="text-lg font-bold"><span className="font-semibold text-muted-foreground">Total Budgeted:</span> GHS {budget.totalBudgetedAmount.toFixed(2)}</div>
          {budget.notes && <div className="md:col-span-3"><span className="font-semibold text-muted-foreground">Notes:</span> {budget.notes}</div>}
        </CardContent>
      </Card>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            {budget.categories.length > 0 ? "Manage your budget categories. Line items within categories are coming soon." : "No categories added yet. Click 'Add Category' to start."}
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Budgeted Amount (GHS)</TableHead>
              ...
              <TableBody>
                {budget.categories.map((category) => (
                  <TableRow key={category.id}>
                    ...
                    <TableCell className="text-right">{category.budgetedAmount.toFixed(2)}</TableCell>
      ...
      <Card className="mt-6 bg-muted/30 p-4">
        <CardHeader className="p-0 pb-2">
            <CardTitle className="text-base font-semibold text-muted-foreground">Next Steps: Line Items</CardTitle>
        </CardHeader>
        <CardContent className="p-0 text-xs text-muted-foreground space-y-1">
            <p>&bull; The next major enhancement for this page will be the ability to add specific line items (e.g., "Seeds", "Fertilizer", "Weeding Labor") under each category.</p>
            <p>&bull; The "Budgeted Amount" for a category will then be automatically calculated as the sum of its line items.</p>
        </CardContent>
      </Card>
    </div>
  );
