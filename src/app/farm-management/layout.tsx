...
export default function FarmManagementLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (isLoading) return;
    if (!user) {
      router.replace('/auth/signin?redirect=/farm-management');
      return;
    }
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[calc(100vh-10rem)] p-4 text-center">
        ...
        <p className="ml-3 mt-4 text-lg text-muted-foreground">Verifying farm operations access...</p>
      </div>
    );
  }
        ...
        <Card className="w-full max-w-lg text-center shadow-lg">
          <CardHeader>
            <CardTitle className="flex items-center justify-center text-xl text-primary">
             ...
             <CardDescription>
                The Farm Operations module is available on the Grower plan and higher.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-muted-foreground mb-4">
              Your current plan is <span className="font-semibold capitalize text-primary">{plan}</span>. Please upgrade your subscription to unlock this feature.
            </p>
